...
function nearestPoints(points: Point[]): Neighbour {
    points.sort((a, b) => a.x - b.x);
    return _find(points, 0, points.length - 1);
    function _find(points: Point[], left: number, right: number): Neighbour {
        /*
 ...
 */
function nearestPointsOptimize(points: Point[]): Neighbour {
    let initYSortPoints = points.slice(0).sort((a, b) => a.y - b.y);
    points.sort((a, b) => a.x - b.x);
    return _find(points, 0, points.length - 1, initYSortPoints);
    function _find(points: Point[], left: number, right: number, ySortPoints: Point[]): Neighbour {
        if (left === right) {
            return {
                points: [],
                distance: Infinity
