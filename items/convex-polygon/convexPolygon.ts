function showCordinary(e: MouseEvent) {
...
function generatePoints(num: number) {
    ...
    return Array.from({ length: num }).map(() => {
        let point = new Point(
            250 + ~~(Math.random() * maxWidth),
            150 + ~~(Math.random() * maxHeight)
        );
        point.draw();
        return point;
    });
...
function generateConvexPolygon4(points: Point[]) {
    let [left, top, right, bottom]: Point[] = [];
    points.sort((a, b) => {
        if (!left || a.x < left.x) {
            left = a;
        }
    ...
    let list = new DLinkedList<Point>();
    ...
    let list2 = new DLinkedList<Point>();
    ...
    function getArc(a: Point, b: Point): number {
 ...
 * 
 */
function generateConvexPolygon5(points: Point[]) {
    let [left, top, right, bottom]: Point[] = [points[0], points[0], points[0], points[0]];
    points.sort((a, b) => {
        if (a.x < left.x) {
            left = a;
        } else if (a.x > right.x) {
    ...
    top = points[0];
    bottom = points[points.length - 1];
    let convexPolygon: Point[] = new Array(points.length * 2 - 1);
        ...
        if (points[i].x <= convexPolygon[head].x) {
            for (let j = head;
                ...
                && (points[i].y === convexPolygon[head].y
                    ...
                    <= getArc(convexPolygon[head], convexPolygon[head + 1]));
                j++) {
                head++;
            }
        ...
        if (points[i].x >= convexPolygon[tail].x) {
            for (let j = tail;
                ...
                && (points[i].y === convexPolygon[tail].y
                    ...
                    >= getArc(convexPolygon[tail], convexPolygon[tail - 1]));
                j--) {
                tail--;
            }
    ...
    let convexPolygon2: Point[] = new Array(points.length * 2 - 1);
        ...
        if (points[i].x <= convexPolygon2[head2].x) {
            for (let j = head2;
                ...
                && (points[i].y === convexPolygon2[head2].y
                    ...
                    >= getArc(convexPolygon2[head2], convexPolygon2[head2 + 1]));
                j++) {
                head2++;
            }
        ...
        if (points[i].x >= convexPolygon2[tail2].x) {
            for (let j = tail2;
                ...
                && (points[i].y === convexPolygon2[tail2].y
                    ...
                    <= getArc(convexPolygon2[tail2], convexPolygon2[tail2 - 1]));
                j--) {
                tail2--;
            }
...
function generateConvexPolygon(points: Point[]) {
    let [left, top, right, bottom]: Point[] = [points[0], points[0], points[0], points[0]];
    points.sort((a, b) => {
        if (a.x < left.x) {
            left = a;
    ...
    function macroTemplate(arcDirection: -1 | 1) {
        if (points[i] === left) {
            meetLeft = true;
        } else if (points[i] === right) {
            meetRight = true;
        ...
        if (points[i].x <= convexPolygon[head].x) {
            for (let j = head;
                ...
                && (points[i].y === convexPolygon[head].y
                    ...
                    <= getArc(convexPolygon[head], convexPolygon[head + 1]) * arcDirection);
                j++) {
                head++;
            }
            convexPolygon[--head] = points[i];
        ...
        if (points[i].x >= convexPolygon[tail].x) {
            for (let j = tail;
                ...
                && (points[i].y === convexPolygon[tail].y
                    ...
                    >= getArc(convexPolygon[tail], convexPolygon[tail - 1]) * arcDirection);
                j--) {
                tail--;
            }
            convexPolygon[++tail] = points[i];
